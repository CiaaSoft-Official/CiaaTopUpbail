...
function printBanner() {
  const ciaatopupGradient = gradient(["#00f2fe", "#4facfe", "#f093fb", "#f5576c"]);
  
  const title = `✨ ${name.toUpperCase()} — BY ${creator.toUpperCase()} ✨`;
  const subtitle = "High Performance WhatsApp Web Library (Multi-Device)";

  console.log();
  console.log(ciaatopupGradient(hr("═")));
  console.log(ciaatopupGradient(centerLine(title).replace("│", "").replace("│", "").trim()));
  console.log(ciaatopupGradient(centerLine(subtitle).replace("│", "").replace("│", "").trim()));
  console.log(ciaatopupGradient(hr("═")));
  console.log();
  ...
  box(infoLines, (s) => ciaatopupGradient(s));
  console.log();
}
